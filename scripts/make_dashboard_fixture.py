...
snaps = [
    ...
    make_snapshot(at=T0 + timedelta(seconds=60), yes_ask=0.55, no_ask=0.55,
                  depth=200.0, event_id="evt-1"),
    # 3. Cross-venue complement: Polymarket YES + Kalshi NO (asks only).
    # 4. Stale snapshot: books 30s older than capture -> rejected.
    make_snapshot(at=T0 + timedelta(seconds=180), yes_ask=0.45, no_ask=0.45,
                  depth=200.0, event_id="evt-1"),
]

# Snapshot 3 built manually for the cross-venue pair.
yv, _ = polymarket_pair(
    yes_ask=0.46, depth=150.0, at=T0 + timedelta(seconds=120), event_id="evt-x")
_, knv = kalshi_yes_no(no_ask=0.44, depth=150.0,
                       at=T0 + timedelta(seconds=120), event_id="evt-x")
# Same normalized question so the deterministic matcher pairs them.
knv.market.question = yv.market.question
cross = SnapshotInput(
    label="simulated", venue=Venue.POLYMARKET,
    markets=[yv.market, knv.market], books=[yv.book, knv.book],
    captured_at=T0 + timedelta(seconds=120))
snaps.insert(2, cross)

# Snapshot 4: stale books.
stale = make_snapshot(at=T0 + timedelta(seconds=240), yes_ask=0.44, no_ask=0.44,
                      depth=200.0, event_id="evt-1",
...
d = report.to_dict()
print(f"executed={d['n_executed']} rejected={d['n_rejected']} "
      f"realized={d['total_realized_net']:.4f}")
print("flags:", d["flags"])
