...
def _two_venue_direct(buy_ask=0.50, sell_bid=0.60, depth=100.0):
    ...
    buy_book = make_book(buy_m, bids=[(buy_ask - 0.02, depth)], asks=[(buy_ask, depth)], at=T0)
    sell_book = make_book(sell_m, bids=[(sell_bid, depth)], asks=[(sell_bid + 0.02, depth)], at=T0)
    return (BookView(buy_m, buy_book, SIM), BookView(sell_m, sell_book, SIM))
...
def test_crossed_book_no_edge_returns_none(config, fees):
    ...
    crossed = make_book(yv.market, bids=[(0.60, 100.0)], asks=[(0.45, 100.0)], at=T0)
    yv = BookView(yv.market, crossed, SIM)
    nv_crossed = make_book(nv.market, bids=[(0.60, 100.0)], asks=[(0.60, 100.0)], at=T0)
...
def test_crossed_book_edge_still_measured_at_ask(config, fees):
    ...
    crossed = make_book(yv.market, bids=[(0.60, 100.0)], asks=[(0.40, 100.0)], at=T0)
    yv = BookView(yv.market, crossed, SIM)
...
def test_cross_venue_direct_crossed_sell_book_no_crash(config, fees):
    ...
    crossed_sell = make_book(sv.market, bids=[(0.60, 100.0)], asks=[(0.55, 100.0)], at=T0)
...
def test_bundle_single_sided_books_detects(config, fees):
    ...
    yv, nv = polymarket_pair(yes_ask=0.45, no_ask=0.45, depth=100.0)
    yv = BookView(yv.market, make_book(yv.market, asks=[(0.45, 100.0)], at=T0), SIM)
    nv = BookView(nv.market, make_book(nv.market, asks=[(0.45, 100.0)], at=T0), SIM)
    opp = detect_bundle_arbitrage(yv, nv, config=config, fee_model=fees, now=T0)
    assert opp is not None
...
def test_complement_single_sided_both_venues_detects(config, fees):
    ...
    opp = detect_cross_venue_complement(pm_yv, knv, config=config, fee_model=fees, now=T0)
    assert opp is not None
...
def test_walk_book_zero_size_levels_do_not_distort_vwap():
    ...
    book = make_book(yv.market, asks=[(0.45, 0.0), (0.45, 50.0), (0.47, 100.0)], at=T0)
...
def test_zero_fee_rate_still_signals(config, fees):
    yv, nv = polymarket_pair(yes_ask=0.45, no_ask=0.45, depth=100.0, taker_fee_rate=0.0)
    opp = detect_bundle_arbitrage(yv, nv, config=config, fee_model=fees, now=T0)
    assert opp is not None
...
def test_fee_exactly_equal_to_edge_signals_no_trade(config, fees):
    ...
    yv, nv = polymarket_pair(yes_ask=0.45, no_ask=0.45, depth=100.0, taker_fee_rate=rate)
    assert detect_bundle_arbitrage(yv, nv, config=config, fee_model=fees, now=T0) is None


def test_fee_exceeding_edge_signals_no_trade(config, fees):
    yv, nv = polymarket_pair(yes_ask=0.45, no_ask=0.45, depth=100.0, taker_fee_rate=0.9)
    assert detect_bundle_arbitrage(yv, nv, config=config, fee_model=fees, now=T0) is None
...
def test_size_cap_below_min_liquidity_returns_none(config, fees):
    yv, nv = polymarket_pair(yes_ask=0.45, no_ask=0.45, depth=100.0)
    assert (
        detect_bundle_arbitrage(yv, nv, config=config, fee_model=fees, now=T0, size_cap=10.0)
        is None
    )
...
def test_size_cap_limits_quantity(config, fees):
    yv, nv = polymarket_pair(yes_ask=0.45, no_ask=0.45, depth=100.0)
    opp = detect_bundle_arbitrage(yv, nv, config=config, fee_model=fees, now=T0, size_cap=60.0)
    assert opp is not None
...
def test_size_cap_zero_means_no_cap_not_zero_size(config, fees):
    ...
    yv, nv = polymarket_pair(yes_ask=0.45, no_ask=0.45, depth=100.0)
    opp = detect_bundle_arbitrage(yv, nv, config=config, fee_model=fees, now=T0, size_cap=0.0)
    assert opp is not None
    assert opp.legs[0]["quantity"] == pytest.approx(100.0)
...
def test_unsorted_asks_rejected():
    ...
    with pytest.raises(ValidationError):
        OrderBook(
            ...
            outcome=m.outcome,
            bids=[],
            asks=[OrderBookLevel(price=0.5, size=10.0), OrderBookLevel(price=0.4, size=10.0)],
        )
...
def test_unsorted_bids_rejected():
    ...
    with pytest.raises(ValidationError):
        OrderBook(
            ...
            venue=m.venue,
            outcome=m.outcome,
            bids=[OrderBookLevel(price=0.4, size=10.0), OrderBookLevel(price=0.5, size=10.0)],
            asks=[],
        )
