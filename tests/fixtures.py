...
def make_book(market: Market, *, bids=None, asks=None, at: datetime | None = None) -> OrderBook:
    at = at or T0
    bid_lvls = [OrderBookLevel(price=p, size=s) for p, s in (bids or [])]
    ask_lvls = [OrderBookLevel(price=p, size=s) for p, s in (asks or [])]
    bid_lvls.sort(key=lambda level: level.price, reverse=True)
    ask_lvls.sort(key=lambda level: level.price)
    return OrderBook(
...
def kalshi_yes_no(
    *, yes_ask=0.45, no_ask=0.45, depth=100.0, at: datetime | None = None, event_id="evt-k1"
):
