...
from backend.arbitrage.opportunities import book_age_seconds
...
class RiskGate:
    ...
    def evaluate(self, opportunity: Opportunity, quantity: float,
        ...
        if net_edge < det.min_net_edge:
            return reject(f"net_edge {net_edge:.4f} < min {det.min_net_edge}")
        checks.append(f"net_edge {net_edge:.4f} >= {det.min_net_edge}")

        if quantity <= 0:
            return reject("sized quantity is zero")
        checks.append(f"quantity {quantity:.2f} > 0")

        if quantity > risk.max_position_per_market:
            return reject(f"quantity {quantity:.2f} > max_position_per_market "
                          f"{risk.max_position_per_market}")
        ...
        if opportunity.strategy == "cross_venue_arbitrage":
            ...
            if conf < self.config.cross_venue.min_match_confidence:
                return reject(f"match_confidence {conf:.2f} below minimum")
