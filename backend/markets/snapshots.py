...
def save_snapshot(path: str | Path, venue: Venue,
                   markets: list[Market], books: list[OrderBook],
                   *, label: str = LABEL_LIVE) -> Path:
    """Persist markets and order books to a labeled JSON snapshot."""
    if label not in (LABEL_LIVE, LABEL_SIMULATED):
        raise ValueError(f"label must be '{LABEL_LIVE}' or '{LABEL_SIMULATED}'")
    payload = {
