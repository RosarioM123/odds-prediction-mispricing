...
class PaperBroker:
    ...
    def execution_latency(self) -> timedelta:
        """Configured execution latency as a timedelta.
    ...
    def execute(
        ...
        Raises:
            None. Missing books, expired markets, and stale quotes are
            modeled as MISSED/EXPIRED trades, never raised.
        """
    ...
    def _fill_leg(
        ...
        *,
        ...
        if market is None or book is None:
            return PaperTrade(
                status=PaperTradeStatus.EXPIRED, simulated_fill_price=leg["expected_price"], **base
            )
        ...
        if (fill_at - book_ts).total_seconds() > self.config.detection.max_book_age_seconds:
            return PaperTrade(
                status=PaperTradeStatus.MISSED, simulated_fill_price=leg["expected_price"], **base
            )

        side = "buy" if leg["side"] == "buy" else "sell"
        walked = walk_book(book, side, quantity)
        vwap = walked.vwap
        if walked.filled <= 0 or vwap is None:
            return PaperTrade(
                status=PaperTradeStatus.MISSED, simulated_fill_price=leg["expected_price"], **base
            )
        fee = self.fee_model.taker_fee(market, walked.filled, vwap)
        # Cash-flow accounting signed by side: buys are outflows, sells inflows.
        signed = -1.0 if side == "buy" else 1.0
        gross = signed * vwap * walked.filled
        status = (
            PaperTradeStatus.FILLED if walked.shortfall <= 0 else PaperTradeStatus.PARTIALLY_FILLED
        )
        return PaperTrade(
            simulated_fill_price=vwap,
            filled_quantity=walked.filled,
            fees=round(fee.fee_dollars, 6),
            slippage=round((vwap - leg["expected_price"]) * walked.filled, 6),
